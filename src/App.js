...
function App() {
  // Số lượng sản phẩm trong giỏ hàng, mỗi sản phẩm sẽ không hiện sẵn
  ...
  const addToCart = (productName) => {
    setCart((prevCart) => ({
      ...prevCart,
      [productName]: (prevCart[productName] || 0) + 1, // Nếu sản phẩm chưa có trong giỏ, khởi tạo với giá trị 1
    }));
  };
  ...
  const decreaseQuantity = (productName) => {
    setCart((prevCart) => {
      if (prevCart[productName] > 1) {
        return {
          ...prevCart,
          [productName]: prevCart[productName] - 1,
        };
      } else {
        // Nếu số lượng là 1 và người dùng bấm giảm thì xóa sản phẩm khỏi giỏ hàng
      ...
      <Modal show={showCartModal} onHide={handleCloseCartModal}>
        ...
        <Modal.Body>
          {Object.keys(cart).length === 0 ? (
            <p>Your shopping cart is empty, please add any product!</p>
          ) : (
